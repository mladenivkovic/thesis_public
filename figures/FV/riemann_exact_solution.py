...
from mesh_hydro_riemann import riemann_solver, soundspeed, find_star_state
...
SR = uR + aR * np.sqrt(0.5*(gamma+1)/gamma * pstar/pR + alpha) # right shock speed
...
for i, ax in enumerate(fig.axes):
    ax.set_xlim(0, 1)
    ax.set_ylim(mins[i], maxs[i])
    ax.plot([xrarehead, xrarehead], [mins[i], maxs[i]], 'r:',  lw=3)
    ax.plot([xraretail, xraretail], [mins[i], maxs[i]], 'r:',  lw=3)
    ax.plot([xcontact, xcontact], [mins[i], maxs[i]], 'g:',  lw=3)
    ax.plot([xshock, xshock], [mins[i], maxs[i]], ':', color='orange',  lw=3)
