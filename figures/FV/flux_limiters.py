...
#----------------------------------------------
# Create r, psi(r) plots for various limiters
#----------------------------------------------
...
ax.annotate(r"$\psi(r)=2r$", [0.3, 1.2], rotation=70)
ax.plot([0, 2.2], [0, 2.2], 'k--', lw=3)
ax.annotate(r"$\psi(r)=r$", [1.2, 1.5], rotation=60)
ax.plot([-1, 5], [1, 1], 'k--', lw=3)
ax.annotate(r"$\psi(r)=1$", [-0.9, 0.88])
ax.plot([-1, 5], [2, 2], 'k--', lw=3)
ax.annotate(r"$\psi(r)=2$", [-0.9, 2.05])



for name, psi in zip(["Minmod", "Superbee", "MC", "van Leer"], [minmod, superbee, MC, vanleer]):
    y = [psi(x) for x in r]
